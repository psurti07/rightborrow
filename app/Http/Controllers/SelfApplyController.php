    ...
    public function buyDigitalPlan(Request $request)
    {
        try {
            ...
            if ($responseCode == 100) {
                ...
                if ($userData->state == 'Gujarat') {
                    $cgstamount = round($netamount * 0.09);
                    $sgstamount = round($netamount * 0.09);
                } else {
        ...
        try {
            ...
            if ($status == 'SUCCESS') {
                ...
                if ($response) {
                    ...
                    if ($regUser) {
                        $converted = convertIntoCustomer($cardno, $regUser, $userData, $paymentdata->orderamount ?? 0, $txnId, 1, 'self-apply', 'SA_', 5);
                        if (!$converted) {
                            Log::error("Conversion to customer failed for user: " . $regUser->id);
                            dd('check log');
                        }
